...
import type { AllocateParams, FlowItem, FlowTotalResponse, UserItem } from '@/views/personal-center/components/my-point/index.api'
...
import { Api } from '@/views/personal-center/components/my-point/index.api'
...
const columns = ref<ProDataTableColumns<FlowItem>>([
  {
    title: $t('myBalance.table.project') as string,
    render: row => row.remark ? row.remark : row.bizTypeName,
  },
  {
    title: $t('myBalance.table.account') as string,
    width: 100,
    path: 'userName',
  },
  {
    title: $t('myBalance.table.time') as string,
    width: 200,
    path: 'createTime',
  },
  {
    title: $t('myBalance.table.incomeExpense') as string,
    width: 100,
    render: row => renderNumber(row),
  },
  {
    title: $t('myBalance.table.actions') as string,
    width: 100,
    render: row => row.bizType === 2 && (
      <n-button
        text
        onClick={() => {
          router.push({
...
async function fetchList(params: any) {
  try {
    const { data } = await Api.getFlowList({ enterpriseId: user.value.enterpriseId!, assetType: 2, pageNum: params.current, pageSize: params.pageSize })
    return { list: data.records, total: Number(data.total) }
  }
  catch (error: any) {
    console.error('获取余额明细失败:', error)
    message.error(error.response?.data?.msg || ($t('myBalance.messages.fetchFail') as string))
...
function recharge() {
  ...
  if (user.value.role === 2) {
    message.warning($t('myBalance.messages.contactManager'))
  }
}
...
async function getUserList() {
  try {
    ...
    message.error(error.response?.data?.msg || ($t('myBalance.messages.fetchUserListFail') as string))
    userList.value = []
  }
}
...
async function handleConfirm() {
  ...
  if (!values.value.amount || values.value.amount <= 0) {
    message.warning($t('myBalance.messages.inputAmount'))
    return
  }

  try {
    const operateType = values.value.flowType === 'increase' ? 3 : 4
    ...
    message.success($t('myBalance.messages.assignSuccess'))
    resetAll()
    useUserStore().fetchUpdateUserInfo()
  }
  catch (error: any) {
    console.error('余额分配失败:', error)
    message.error(error.response?.data?.msg || ($t('myBalance.messages.assignFail') as string))
  }
}
...
async function getFlowTotal() {
  try {
    ...
    message.error(error.response?.data?.msg || ($t('myBalance.messages.fetchTotalFail') as string))
  }
}
        ...
        <template v-if="isManager">
          <div class="flex flex-col">
            <div class="text-12px text-#606060">
              {{ $t('myBalance.labels.totalBalance') }}
            </div>
            <div class="text-33px text-#000000 font-bold font-din">
              {{ user.enterpriseTotalBalance }}
            </div>
          </div>
          <div class="flex flex-col">
            <div class="text-12px text-#606060">
              {{ $t('myBalance.labels.allocatableBalance') }}
            </div>
            <div class="text-33px text-#000000 font-bold font-din">
              {{ user.enterpriseUnallocatedBalance }}
            </div>
          </div>
          <div class="flex flex-col">
            <div class="text-12px text-#606060">
              {{ $t('myBalance.labels.employeeTotalBalance') }}
            </div>
            <div class="text-33px text-#000000 font-bold font-din">
              {{ Number(user.enterpriseTotalBalance) - Number(user.enterpriseUnallocatedBalance) }}
            </div>
          </div>
        </template>
        <template v-else>
    ...
    <div class="flex flex-col flex-1 rounded-16px p-24px box-border gap-16px bg-#FFFFFF">
      <div class="w-full flex items-center justify-between">
        <div class="c-#000000 text-16px font-bold font-['PingFang_SC']">
          {{ $t('myBalance.labels.detail') }}
        </div>
        <div class="flex gap-6px">
          <div class="font-['PingFang_SC']">
            {{ $t('myBalance.labels.income') }}:
            <span class="c-#FF4E47">
              {{ totalFlowInfo.balanceIncome }}
            </span>
          </div>
          <div class="font-['PingFang_SC']">
            {{ $t('myBalance.labels.expense') }}:
            <span class="c-#19C906">
              {{ totalFlowInfo.balanceExpense }}
            </span>
          </div>
        </div>
      </div>
    ...
    <n-scrollbar class="w-full h-280px px-5px">
      <div class="size-full w-95% flex flex-col gap-8px px-5px">
        <div class="font-['PingFang_SC']">
          ...
          {{ $t('myBalance.modal.selectEmployee') }}
        </div>
        <n-config-provider
          :theme-overrides="{
            common: {
        ...
        <div class="mt-4px flex-col flex rounded-8px bg-#F7F7F7 p-8px">
          <div class="text-12px c-#606060">
            {{ $t('myBalance.modal.employeeAvailableBalance') }}：<span class="c-#000">{{ selectedUserBalance }}</span>
          </div>
          <div class="text-12px c-#606060">
            {{ $t('myBalance.modal.allocatableBalance') }}：<span class="c-#000">{{ user.enterpriseUnallocatedBalance }}</span>
          </div>
        </div>
        <div class="mt-8px">
          <span class="c-red">*</span>
          {{ $t('myBalance.modal.assign') }}
        </div>
        <n-input-group>
          <n-select
            ...
            :style="{ width: '33%' }"
            :options="[
              { label: $t('myBalance.modal.increase'), value: 'increase' },
              { label: $t('myBalance.modal.decrease'), value: 'decrease' },
            ]"
          />
          <n-input-number
            ...
            :style="{ width: '66%' }"
            :min="0"
            :placeholder="$t('myBalance.modal.amountPlaceholder')"
          />
        </n-input-group>
        <div class="mt-8px">
          {{ $t('myBalance.modal.remark') }}
        </div>
        <n-input
          v-model:value="values.remark"
          :placeholder="$t('myBalance.modal.remarkPlaceholder')"
          class="mb-10px"
        />
      </div>
    </n-scrollbar>
    <template #footer>
      <div class="w-full flex justify-end gap-x-8px">
        ...
        <n-button
          ...
          @click="handleConfirm"
        >
          {{ $t('myBalance.modal.confirmAssign') }}
        </n-button>
      </div>
    </template>
  </n-modal>

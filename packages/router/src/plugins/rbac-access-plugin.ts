...
export function rbacAccessPlugin(
  ...
  return ({ router, onUnmount }) => {
    ...
    router.beforeEach(async (to, from) => {
      ...
      const { logined, homePath, loginPath, handleUnLoginedAction }
        = resolvedOptions
      ...
      // 未登录重定向到登录页
      if (!logined) {
        if (handleUnLoginedAction) {
          return handleUnLoginedAction(from, to)
        }
        else {
          return {
            path: loginPath,
...
async function resolveOptions(
  ...
  return {
    ...
    handleUnLoginedAction: handleUnLoginedAction ?? null,
  }
}
...
async function resolveRoutes(
  ...
  const { mode, parentNameForAddRoute } = options
